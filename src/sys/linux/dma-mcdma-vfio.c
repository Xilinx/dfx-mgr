...
static int mcdma_vfio_dma_transfer(acapd_chnl_t *chnl, acapd_dma_config_t *config)
{
	acapd_device_t *dev;
	...
	for (; transfer_buf_size != 0;) {
		...
		*((volatile uint32_t *)((char *)next_bd_va + 0x14)) =
			(tmpbuf_size & 0x3FFFFFF) | 0x80000000 | 0x40000000;
	...
	if (chnl->dir == ACAPD_DMA_DEV_W) {
		...
		if (((rx_status & MCDMA_HALTED_MASK) && (rx_error != 0)) || (rx_status & MCDMA_IDLE_MASK)) {
		...
		*((volatile uint32_t *)((char *)base_va + offset + MM2S_CH1CURDESC_LSB)) =
			bd_pa & 0xFFFFFFC0;
		...
		*((volatile uint32_t *)((char *)base_va + offset + MM2S_CH1TAILDESC_LSB)) =
			next_bd_pa & 0xFFFFFFC0;
		*((volatile uint32_t *)((char *)base_va + offset + MM2S_CH1TAILDESC_MSB)) =
			next_bd_pa >> 32;
		...
		if (((tx_status & MCDMA_HALTED_MASK) && (tx_error != 0)) || (tx_status & MCDMA_IDLE_MASK)) {
		...
		*((volatile uint32_t *)((char *)base_va + offset + S2MM_CH1CURDESC_LSB)) =
			bd_pa & 0xFFFFFFC0;
		...
		*((volatile uint32_t *)((char *)base_va + S2MM_CR)) = MCDMA_RUNSTOP_MASK;
		/* program Tail desc. Each channel needs two desc-for data and control each*/
		*((volatile uint32_t *)((char *)base_va + offset + S2MM_CH1TAILDESC_LSB)) =
			next_bd_pa & 0xFFFFFFC0;
		*((volatile uint32_t *)((char *)base_va + offset + S2MM_CH1TAILDESC_MSB)) =
			next_bd_pa >> 32;
	}
	return buf_size;
}
