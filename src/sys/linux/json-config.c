...
int parseShellJson(acapd_shell_t *shell, const char *filename)
{
	...
	for (i = 1; i < ret; i++) {
		...
		if (jsoneq(jsonData, &token[i], "clock_reg_base") == 0)
			clk_dev->reg_pa = (uint64_t)strtol(
				strndup(jsonData + token[i + 1].start, token[i + 1].end - token[i + 1].start), NULL,
				16);
		if (jsoneq(jsonData, &token[i], "clock_reg_size") == 0)
			clk_dev->reg_size = (size_t)strtol(
				strndup(jsonData + token[i + 1].start, token[i + 1].end - token[i + 1].start), NULL,
				16);
	}
	free(jsonData);
...
int initAccel(accel_info_t *accel, const char *path)
{
	...
	for (i = 1; i < ret; i++) {
		...
		if (jsoneq(jsonData, &token[i], "accel_type") == 0) {
			int sz = MIN(token[i + 1].end - token[i + 1].start, sizeof(accel->accel_type) - 1);
			strncpy(accel->accel_type, jsonData + token[i + 1].start, sz);
	...
	for (i = 1; i < ret; i++) {
		...
		if (jsoneq(jsonData, &token[i], "firmware_location") == 0) {
			...
			config->firmware_locations =
				(char **)calloc(token[i + 1].size, MAX_PATH_SIZE * sizeof(char));
			for (k = 0; k < token[i + 1].size; k++) {
				config->firmware_locations[k] =
					strndup(jsonData + token[i + k + 2].start,
