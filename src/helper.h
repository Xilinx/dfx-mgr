...
#define ACAPD_INIT_LIST(name)        \
	{                                \
		.next = &name, .prev = &name \
	}
#define ACAPD_DECLARE_LIST(name) acapd_list_t name = ACAPD_INIT_LIST(name)
...
static inline void acapd_list_add_before(acapd_list_t *node, acapd_list_t *new_node)
{
...
static inline void acapd_list_add_after(acapd_list_t *node, acapd_list_t *new_node)
{
	new_node->prev = node;
...
static inline void acapd_list_add_head(acapd_list_t *list, acapd_list_t *node)
{
	acapd_list_add_after(list, node);
}

static inline void acapd_list_add_tail(acapd_list_t *list, acapd_list_t *node)
{
...
#define acapd_list_for_each_safe(list, node, tmp)                       \
	for ((node) = (list)->next, (tmp) = (node)->next; (node) != (list); \
		 (node) = (tmp), (tmp) = (node)->next)

#define offset_of(structure, member) ((uintptr_t) & (((structure *)0)->member))
...
#define acapd_align_down(size, align) ((size) & ~((align)-1))

/** Align 'size' up to a multiple of 'align' (must be a power of two). */
#define acapd_align_up(size, align) acapd_align_down((size) + (align)-1, align)
