...
int sihaDma_MM2SData(DeviceConfig_t *config, BuffConfig_t *buffer, uint64_t offset, uint64_t size, uint8_t firstLast, uint8_t tid){
	...
	_unused(firstLast);
	acapd_debug("interRMEn %d", buffer->interRMEnabled);
	acapd_debug("slot %d", config->slot);
	acapd_debug("sinkSlot %d", buffer->sinkSlot);
	acapd_debug("sourceSlot %d", buffer->sourceSlot);
	if (buffer->interRMEnabled){
		address = buffer->interRMSinks[buffer->sourceSlot] + offset;
	}
	else{
		address = buffer->phyAddr + offset;
	}
...
int sihaDma_S2MMData(DeviceConfig_t *config, BuffConfig_t *buffer, uint64_t offset, uint64_t size, uint8_t firstLast){
	...
	acapd_debug("sourceSlot %d", buffer->sourceSlot);
	if (buffer->interRMEnabled){
		address = buffer->interRMSinks[config->slot] + offset;
	}
	else{
		address = buffer->phyAddr + offset;
	}
